...
import { MessageSquare, Star, Send, Trash2, Reply, ChevronDown, ChevronUp } from "lucide-react";
...
export default function CommentSection({ animeId }: CommentSectionProps) {
  ...
  const fetchComments = async () => {
    try {
      const res = await fetch(`/api/comments?animeId=${animeId}&limit=50`);
      if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/comments", {
        ...
        body: JSON.stringify({ animeId, text, rating: rating || undefined }),
      });
      if (res.ok) {
  ...
  const handleReply = async (parentId: string) => {
    ...
    try {
      const res = await fetch("/api/comments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ animeId, text: replyText, parentId }),
      });
      if (res.ok) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-foreground">评论</h2>
      ...
      <div className="p-4 rounded-xl bg-card border border-border space-y-3">
        ...
        <div className="flex items-center gap-1">
          <span className="text-sm text-muted mr-2">评分：</span>
          {[1, 2, 3, 4, 5].map(star => (
            <button
              key={star}
            ...
            >
              <Star className={`w-5 h-5 transition-colors ${
                star <= rating ? "fill-yellow-400 text-yellow-400" : "text-muted hover:text-yellow-300"
              }`} />
            </button>
          ))}
          {rating > 0 && <span className="text-sm text-yellow-400 ml-1">{rating}分</span>}
        </div>
        ...
        <div className="flex gap-3">
          <textarea
            ...
            placeholder={session?.user ? "说点什么吧..." : "登录后即可评论..."}
